...
#[allow(dead_code)]
pub const DBC_FILE_NAME: &str = "pdm20";
...
impl Messages {
    ...
    pub fn from_can_message(id: Id, payload: &[u8]) -> Result<Self, CanError> {
        let res = match id {
            ...
            AnalogInputs::MESSAGE_ID => {
                Messages::AnalogInputs(AnalogInputs::try_from(payload)?)
            }
            Power::MESSAGE_ID => Messages::Power(Power::try_from(payload)?),
            CurrentSense::MESSAGE_ID => {
                Messages::CurrentSense(CurrentSense::try_from(payload)?)
            }
...
impl Control {
    pub const MESSAGE_ID: embedded_can::Id = Id::Extended(unsafe {
        ExtendedId::new_unchecked(0x18ef5500)
    });
...
impl Configure {
    pub const MESSAGE_ID: embedded_can::Id = Id::Extended(unsafe {
        ExtendedId::new_unchecked(0x19ef5000)
    });
    pub const MUX_MIN: u8 = 0_u8;
    pub const MUX_MAX: u8 = 15_u8;
    ...
    pub fn mux(&mut self) -> Result<ConfigureMuxIndex, CanError> {
        match self.mux_raw() {
            ...
            multiplexor => {
                Err(CanError::InvalidMultiplexor {
                    message_id: Configure::MESSAGE_ID,
                    multiplexor: multiplexor.into(),
                })
            }
        }
...
impl Startup {
    ...
    pub fn new(
        ...
        hardware_version_patch: u8,
        software_major: u8,
        software_minor: u8,
        software_patch: u8,
        reset_cause_brownout: bool,
        reset_cause_watchdog: bool,
        reset_cause_software: bool,
...
impl defmt::Format for Startup {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            ...
            self.hardware_version_major(), self.hardware_version_minor(), self
            .hardware_version_patch(), self.software_major(), self.software_minor(), self
            .software_patch(), self.reset_cause_brownout(), self.reset_cause_watchdog(),
...
impl SystemStatus {
    pub const MESSAGE_ID: embedded_can::Id = Id::Standard(unsafe {
        StandardId::new_unchecked(0x1255)
    });
...
impl defmt::Format for SystemStatus {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f, "SystemStatus {{ CAN_RX_Errors={:?} CAN_TX_Errors={:?} }}", self
            .can_rx_errors(), self.can_tx_errors(),
        );
    }
}
/// Analog_Inputs
...
impl AnalogInputs {
    pub const MESSAGE_ID: embedded_can::Id = Id::Standard(unsafe {
        StandardId::new_unchecked(0x1150)
    });
    ...
    pub fn set_input_1(&mut self, value: u16) -> Result<(), CanError> {
        if value < 0_u16 || 65535_u16 < value {
            return Err(CanError::ParameterOutOfRange {
                message_id: AnalogInputs::MESSAGE_ID,
            });
        }
        let factor = 1;
        let value = value
...
impl defmt::Format for AnalogInputs {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f, "AnalogInputs {{ Input_1={:?} Input_2={:?} Input_3={:?} }}", self
            .input_1(), self.input_2(), self.input_3(),
        );
    }
}
/// Power
...
impl Power {
    pub const MESSAGE_ID: embedded_can::Id = Id::Standard(unsafe {
        StandardId::new_unchecked(0x1250)
    });
...
impl defmt::Format for Power {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f, "Power {{ Bus_Voltage={:?} Bus_Current={:?} Temperature={:?} }}", self
            .bus_voltage(), self.bus_current(), self.temperature(),
        );
    }
}
/// Current_Sense
...
impl CurrentSense {
    pub const MESSAGE_ID: embedded_can::Id = Id::Standard(unsafe {
        StandardId::new_unchecked(0x1350)
    });
    pub const MUX_MIN: u8 = 0_u8;
    pub const MUX_MAX: u8 = 15_u8;
    ...
    pub fn mux(&mut self) -> Result<CurrentSenseMuxIndex, CanError> {
        match self.mux_raw() {
            ...
            multiplexor => {
                Err(CanError::InvalidMultiplexor {
                    message_id: CurrentSense::MESSAGE_ID,
                    multiplexor: multiplexor.into(),
                })
            }
        }
