/* The following is taken from the cortex-m-rt crate with some modifications to add our image
   header section. */
...
SECTIONS
{
  ...
  {
    KEEP(*(.header));
    ASSERT(. == ALIGN(_HEADER_ALIGN), "error: header alignment is invalid");
    HEADER = .;
    . = . + _HEADER_SIZE;
  } > FLASH
...
/* Default interrupt handlers provided by stm32-metapac */
INCLUDE device.x
