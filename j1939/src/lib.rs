...
use heapless::Vec;
use message::{AbortReason, AbortSenderRole};

#[derive(Debug, Clone)]
pub enum TransferResponse {
    Cts(message::ClearToSend),
    End(message::EndOfMessageAck),
}
...
impl<const N: usize> Transfer<N> {
    ...
    pub fn new(rts: message::RequestToSend) -> Self {
    ...
    pub fn data_transfer(
        &mut self,
        msg: message::DataTransfer,
    ) -> Result<Option<TransferResponse>, message::ConnectionAbort> {
        // confirm sequence
        if msg.sequence() != self.rx_packets + 1 {
            return Err(message::ConnectionAbort::new(
        ...
        if self.buffer.is_full() {
            return Ok(Some(TransferResponse::End(message::EndOfMessageAck::new(
                self.rts.total_size(),
        ...
        if let Some(packets_per_response) = self.rts.max_packets_per_response() {
            ...
            if msg.sequence() % packets_per_response == 0 {
                return Ok(Some(TransferResponse::Cts(message::ClearToSend::new(
...
mod tests {
    ...
    #[test]
    fn transmission() {
        let rts = message::RequestToSend::new(16, Some(2), id::Pgn::ProprietaryA);
